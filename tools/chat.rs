...
use ane::mil::{projection, ffn};
use ane::surface::{AneSurface, fp16_to_f32, f32_to_fp16, cvt_f16_f32};
use ane::AneModel;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let encoding = tokenizer.encode(prompt.as_str(), false).map_err(|e| format!("Tokenize: {}", e))?;
    ...
    for l in 0..cfg.n_layers {
        let mut t = vec![0.0f32; cfg.wq_size()]; weights::transpose(&mut t, &layers[l].wq, cfg.q_dim, cfg.dim); wqt.push(t);
        let mut t = vec![0.0f32; cfg.wk_size()]; weights::transpose(&mut t, &layers[l].wk, cfg.kv_dim, cfg.dim); wkt.push(t);
        let mut t = vec![0.0f32; cfg.wv_size()]; weights::transpose(&mut t, &layers[l].wv, cfg.kv_dim, cfg.dim); wvt.push(t);
        let mut t = vec![0.0f32; cfg.wo_size()]; weights::transpose(&mut t, &layers[l].wo, cfg.dim, cfg.q_dim); wot.push(t);
        let mut t = vec![0.0f32; cfg.w1_size()]; weights::transpose(&mut t, &layers[l].w1, cfg.hidden, cfg.dim); w1t.push(t);
        let mut t = vec![0.0f32; cfg.w3_size()]; weights::transpose(&mut t, &layers[l].w3, cfg.hidden, cfg.dim); w3t.push(t);
    }
    ...
    let mut qkv_model = AneModel::compile(&qkv_prog, &[])?; qkv_model.load()?;
    let mut wo_model = AneModel::compile(&wo_prog, &[])?; wo_model.load()?;
    let mut ffn_model = AneModel::compile(&ffn_prog, &[])?; ffn_model.load()?;
    eprintln!(" {}ms", t0.elapsed().as_millis());

    // Allocate surfaces, stage weights, KV cache
    let mut qkv_ins = Vec::new(); let mut wo_ins = Vec::new(); let mut ffn_ins = Vec::new();
    ...
    for l in 0..cfg.n_layers {
        ...
        stage_qkv(&qi, &wqt[l], &wkt[l], &wvt[l], &cfg);
        stage_wo(&wi, &wot[l], &cfg);
        stage_ffn(&fi, &w1t[l], &w3t[l], &layers[l].w2, &cfg);
        qkv_ins.push(qi); wo_ins.push(wi); ffn_ins.push(fi);
    }
    drop(wqt); drop(wkt); drop(wvt); drop(wot); drop(w1t); drop(w3t);

    let mut kvc: Vec<weights::KVCache> = (0..cfg.n_layers).map(|_| weights::KVCache::alloc(&cfg)).collect();
    ...
    for (i, &t) in token_ids.iter().enumerate().take(prompt_len) { tokens[i] = t as u16; }
    ...
    let mut next = sample(&logits, cfg.vocab, temperature, topk);
    ...
    for g in 1..maxlen.min(cfg.seq - prompt_len) {
        ...
        decode_step(&mut x_single, &mut logits, pos, &layers, &rms_final, &embed_w, &mut kvc, &cfg, &rope_cos, &rope_sin);

        next = sample(&logits, cfg.vocab, temperature, topk);
        generated.push(next as u32);
        print_token(&tokenizer, &generated);
        if next == 151643 || next == 151645 { break; }
        if prompt_len + g < cfg.seq { tokens[prompt_len + g] = next as u16; }
        else { break; }
    }
    println!();
...
fn sample(logits: &[f32], v: usize, temp: f32, topk: usize) -> usize {
    if temp <= 0.0 {
        logits[..v].iter().enumerate().max_by(|a, b| a.1.partial_cmp(b.1).unwrap()).unwrap().0
    } else {
        let k = topk.min(v);
        ...
        let mut sum = 0.0f32;
        for v in &mut val { *v = ((*v - max_v) / temp).exp(); sum += *v; }
        for v in &mut val { *v /= sum; }
        let r = rand_f32();
        let mut cum = 0.0f32;
        for i in 0..k { cum += val[i]; if r < cum { return idx[i]; } }
        idx[k - 1]
    }
}
...
fn rand_f32() -> f32 {
    ...
    static S: AtomicU64 = AtomicU64::new(0xDEADBEEF42);
...
fn prefill(x: &mut [f32], layers: &[weights::LayerWeights], rms_final: &[f32],
    ...
    for l in 0..cfg.n_layers {
        ...
        qkv_out.with_data(|buf| {
            cvt_f16_f32(&mut q, &buf[..q_dim*seq]);
            cvt_f16_f32(&mut k, &buf[q_dim*seq..(q_dim+kv_dim)*seq]);
            cvt_f16_f32(&mut v, &buf[(q_dim+kv_dim)*seq..(q_dim+2*kv_dim)*seq]);
        });
        ...
        write_wo_acts(&wo_ins[l], &attn, cfg);
        wo_m.run(&wo_ins[l], wo_out).unwrap();
        wo_out.with_data(|buf| cvt_f16_f32(&mut o, &buf[..dim*seq]));
        unsafe { ane::accel::vDSP_vadd(x.as_ptr(), 1, o.as_ptr(), 1, x2.as_mut_ptr(), 1, (seq*dim) as u64); }
        rmsnorm::forward(&mut x2n, &x2, &layers[l].rms_ffn, dim, seq);
        write_ffn_acts(&ffn_ins[l], &x2n, &x2, cfg);
...
fn decode_step(x: &mut [f32], logits: &mut [f32], pos: usize,
    ...
    let mut q = vec![0.0f32; q_dim]; let mut k = vec![0.0f32; kv_dim]; let mut v = vec![0.0f32; kv_dim];
    let mut ao = vec![0.0f32; q_dim]; let mut oo = vec![0.0f32; dim];
    let mut x2 = vec![0.0f32; dim]; let mut x2n = vec![0.0f32; dim];
    let mut h1 = vec![0.0f32; hidden]; let mut h3 = vec![0.0f32; hidden]; let mut gate = vec![0.0f32; hidden];
    ...
    for l in 0..cfg.n_layers {
        ...
        unsafe {
            ane::accel::cblas_scopy(kv_dim as i32, k.as_ptr(), 1, kvc[l].k_cache.as_mut_ptr().add(pos), cfg.seq as i32);
            ane::accel::cblas_scopy(kv_dim as i32, v.as_ptr(), 1, kvc[l].v_cache.as_mut_ptr().add(pos), cfg.seq as i32);
        }
        attention::cpu_attention_cached(&mut ao, &q, &kvc[l].k_cache, &kvc[l].v_cache, cfg.heads, cfg.kv_heads, hd, pos+1, cfg.seq);
        ane::accel::sgemv(false, dim, q_dim, 1.0, &layers[l].wo, q_dim, &ao, 0.0, &mut oo);
        unsafe { ane::accel::vDSP_vadd(x.as_ptr(), 1, oo.as_ptr(), 1, x2.as_mut_ptr(), 1, dim as u64); }
        ...
        unsafe {
            let h = hidden as u64; let n = hidden as i32; let neg1 = -1.0f32;
        ...
        unsafe { ane::accel::vDSP_vadd(x2.as_ptr(), 1, oo.as_ptr(), 1, x.as_mut_ptr(), 1, dim as u64); }
    }
...
fn apply_rope(x: &mut [f32], pos: usize, dim: usize, hd: usize, half_hd: usize, cos_t: &[f32], sin_t: &[f32]) {
    let nheads = dim / hd;
...
// ── IOSurface staging (same as infer.rs) ──

fn stage_qkv(s: &AneSurface, wq: &[f32], wk: &[f32], wv: &[f32], c: &config::ModelConfig) {
    let sp = c.qkv_proj_sp();
    s.with_data_mut(|buf| {
        for d in 0..c.dim {
            for i in 0..c.q_dim { buf[d*sp+c.seq+i] = f32_to_fp16(wq[d*c.q_dim+i]); }
            for i in 0..c.kv_dim { buf[d*sp+c.seq+c.q_dim+i] = f32_to_fp16(wk[d*c.kv_dim+i]); }
            for i in 0..c.kv_dim { buf[d*sp+c.seq+c.q_dim+c.kv_dim+i] = f32_to_fp16(wv[d*c.kv_dim+i]); }
        }
    });
}
fn stage_wo(s: &AneSurface, wo: &[f32], c: &config::ModelConfig) {
    let sp = c.wo_fwd_sp();
    s.with_data_mut(|buf| { for d in 0..c.q_dim { for i in 0..c.dim { buf[d*sp+c.seq+i] = f32_to_fp16(wo[d*c.dim+i]); } } });
}
fn stage_ffn(s: &AneSurface, w1: &[f32], w3: &[f32], w2: &[f32], c: &config::ModelConfig) {
    let sp = c.ffn_fused_sp(); let (seq, h, dim) = (c.seq, c.hidden, c.dim);
    s.with_data_mut(|buf| {
        for d in 0..dim {
            for i in 0..h { buf[d*sp+2*seq+i] = f32_to_fp16(w1[d*h+i]); }
            for i in 0..h { buf[d*sp+2*seq+h+i] = f32_to_fp16(w3[d*h+i]); }
            for i in 0..h { buf[d*sp+2*seq+2*h+i] = f32_to_fp16(w2[d*h+i]); }
        }
    });
}
fn write_qkv_acts(s: &AneSurface, xn: &[f32], c: &config::ModelConfig) {
    let sp = c.qkv_proj_sp();
    s.with_data_mut(|buf| { for d in 0..c.dim { for t in 0..c.seq { buf[d*sp+t] = f32_to_fp16(xn[d*c.seq+t]); } } });
}
fn write_wo_acts(s: &AneSurface, a: &[f32], c: &config::ModelConfig) {
    let sp = c.wo_fwd_sp();
    s.with_data_mut(|buf| { for d in 0..c.q_dim { for t in 0..c.seq { buf[d*sp+t] = f32_to_fp16(a[d*c.seq+t]); } } });
}
fn write_ffn_acts(s: &AneSurface, x2n: &[f32], x2: &[f32], c: &config::ModelConfig) {
    let sp = c.ffn_fused_sp(); let seq = c.seq;
    s.with_data_mut(|buf| {
        for d in 0..c.dim { for t in 0..seq { buf[d*sp+t] = f32_to_fp16(x2n[d*seq+t]); buf[d*sp+seq+t] = f32_to_fp16(x2[d*seq+t]); } }
    });
}
