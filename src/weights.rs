...
pub struct LayerWeights {
    ...
    pub w1: Vec<f32>, pub w2: Vec<f32>, pub w3: Vec<f32>,
    pub rms_att: Vec<f32>, pub rms_ffn: Vec<f32>,
...
impl LayerWeights {
    pub fn alloc(cfg: &ModelConfig) -> Self {
        LayerWeights {
            wq: vec![0.0; cfg.wq_size()], wk: vec![0.0; cfg.wk_size()],
            wv: vec![0.0; cfg.wv_size()], wo: vec![0.0; cfg.wo_size()],
            w1: vec![0.0; cfg.w1_size()], w2: vec![0.0; cfg.w2_size()],
            w3: vec![0.0; cfg.w3_size()],
            rms_att: vec![0.0; cfg.dim], rms_ffn: vec![0.0; cfg.dim],
            q_norm: vec![1.0; cfg.hd], k_norm: vec![1.0; cfg.hd],
        }
    }
}
...
pub fn load_checkpoint(
    ...
    f.read_exact(&mut hdr_bytes).map_err(|e| format!("Read header: {}", e))?;
    let hdr: CkptHeader = unsafe { std::ptr::read(hdr_bytes.as_ptr() as *const CkptHeader) };

    if hdr.magic != CKPT_MAGIC { return Err(format!("Bad magic: {:#x}", hdr.magic)); }
    if hdr.version != 4 && hdr.version != 5 { return Err(format!("Bad version: {}", hdr.version)); }
    if hdr.n_layers as usize != cfg.n_layers || hdr.dim as usize != cfg.dim {
        return Err("Model config mismatch".into());
    }

    let v5 = hdr.version >= 5;
    let mut adam_skip: usize = 2 * (cfg.wq_size() + cfg.wk_size() + cfg.wv_size() + cfg.wo_size()
        + cfg.w1_size() + cfg.w2_size() + cfg.w3_size() + cfg.dim + cfg.dim);
    if v5 { adam_skip += 2 * (cfg.hd + cfg.hd); }
    adam_skip *= 4; // f32 bytes

    let mut layers = Vec::with_capacity(cfg.n_layers);
    for _ in 0..cfg.n_layers {
        let mut lw = LayerWeights::alloc(cfg);
        read_f32(&mut f, &mut lw.wq)?; read_f32(&mut f, &mut lw.wk)?;
        read_f32(&mut f, &mut lw.wv)?; read_f32(&mut f, &mut lw.wo)?;
        read_f32(&mut f, &mut lw.w1)?; read_f32(&mut f, &mut lw.w2)?;
        read_f32(&mut f, &mut lw.w3)?;
        read_f32(&mut f, &mut lw.rms_att)?; read_f32(&mut f, &mut lw.rms_ffn)?;
        if v5 {
            read_f32(&mut f, &mut lw.q_norm)?; read_f32(&mut f, &mut lw.k_norm)?;
        }
        f.seek(SeekFrom::Current(adam_skip as i64)).map_err(|e| e.to_string())?;
        layers.push(lw);
    }
    ...
    f.seek(SeekFrom::Current((2 * cfg.dim * 4) as i64)).map_err(|e| e.to_string())?;
...
fn read_f32(f: &mut std::fs::File, buf: &mut [f32]) -> Result<(), String> {
    let bytes = unsafe { std::slice::from_raw_parts_mut(buf.as_mut_ptr() as *mut u8, buf.len() * 4) };
