...
pub fn cross_entropy(dlogits: &mut [f32], logits: &[f32], targets: &[u16], v: usize, s: usize) -> f32 {
    ...
    for t in 0..s {
        ...
        unsafe { cblas_scopy(v as i32, logits.as_ptr().add(t), s as i32, col.as_mut_ptr(), 1); }
        ...
        col[tgt] -= 1.0;
        unsafe { vDSP_vsmul(col.as_ptr(), 1, &inv_s, col.as_mut_ptr(), 1, v as u64); }

        // Scatter back
        unsafe { cblas_scopy(v as i32, col.as_ptr(), 1, dlogits.as_mut_ptr().add(t), s as i32); }
    }
    total_loss / s as f32
}
